...
export async function getAssignmentsForTeacher(classId?: string) {
  ...
  let query = supabase
    .from("assignments")
    .select(
      "id, class_id, title, type, due_date, points, published, created_at, " +
        "submissions(count), classes(name, period_number)"
    )
...
export async function getSubmissionForGrading(submissionId: string) {
  ...
  const { data } = await supabase
    ...
    .select(
      "*, users(first_name, last_name, username), " +
        "assignments(title, type, instructions, points, due_date, class_id), " +
...
export async function getAssignmentsForStudent(userId: string) {
  ...
  // Get assignments in classes the student is enrolled in
  ...
  // Get this student's submissions to overlay status
